...
function Firework(centerX, centerY, color) {
    ...
    for (var i = 0; i < this.particles.length; i++) {
        this.particles[i] = new Particle(
            ...
            /* r= */ 0, /* T= */ r * Math.random(), /* o= */ r * Math.random(),
            /* size= */ 2, color
        );
    }
}
...
Firework.prototype.update = function update() {
    for (var i = 0; i < this.particles.length; i++) {
        this.particles[i].r += this.Dr;
        this.particles[i].recalcCartesianProjection();

        this.Dr -= 0.00005 * this.Dr * this.Dr;                     // Air resist
...
function Particle(x, y, r, T, o, size, color) {
    this.origX = x;
    this.origY = y;
    this.r = r;
    this.sinT = Math.sin(T);
    // this.cosT = Math.cos(T);         // Not needed
    this.sino = Math.sin(o);
    this.coso = Math.cos(o);
    this.size = size;
    this.color = color;
...
Particle.prototype.recalcCartesianProjection = function() {
    this.x = this.origX + this.r * this.sinT * this.coso;
    this.y = this.origY + this.r * this.sinT * this.sino;
};